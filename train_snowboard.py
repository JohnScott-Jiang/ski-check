...
class Net96x96(nn.Module):
    def __init__(self):
        super(Net96x96, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 64, 3, 1, 1),  # 32x96x96
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(64, 64, 3, 1, 1),  # 64x48x48
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.Conv2d(64, 64, 3, 1, 1),  # 64x48x48
            nn.BatchNorm2d(64)
        ...
        self.dense = nn.Sequential(
            nn.Linear(64 * 6 * 6, 128),  # fc4 64*3*3 -> 128
            nn.ReLU(),
            nn.Linear(128, 3)  # fc5 128->10
        )

    def forward(self, x):
        conv1_out = self.conv1(x) # 32x48x48
        conv2_out = self.conv2(conv1_out) + conv1_out
        conv2_out = self.pool(conv2_out)# 64x24x24
        conv3_out = self.conv3(conv2_out) + conv2_out
        conv3_out = self.pool(conv3_out)# 64x12x12
        conv4_out = self.conv4(conv3_out) + conv3_out
        conv4_out = self.pool(conv4_out)# 64x6x6
